...
module.exports.get = (req, res, next) => {
  ...
  try{
    Model.findById(id)
      ...
      .catch(err => res.status(500).json({message: "Error occur in Model catch block", error: err.message}))
  } catch(error){
    console.log(error)
    res.status(500).json({message:"Error in try-catch block", error})
  }
}
...
module.exports.delete = async (req, res, next) => {
  ...
  if(!checkDependency.length){
    Model.findByIdAndDelete(id, (err, data) => {
      ...
      if(data){
        res.status(200).json({
          message: `Удален отдел: ${data.name}`,
          data
        })
      } else {
        res.status(404).json({
